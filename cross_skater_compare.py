...
# Only these two are safe to pull from pre-extracted CSVs as a fallback when
# no video is available. Knee flexion angle is computed from 3 landmark
# points and is inherently scale-invariant (doesn't depend on camera
# distance/zoom), so it's directly comparable across skaters WITHOUT
# needing bone-length normalization.
#
# Hip/shoulder POSITION columns are deliberately NOT pulled from CSVs here,
# even when present (e.g. Ragne Wiklund's CSV has right_hip_x etc.), because
# those were never passed through compute_video_reference_scale() -- we
# cannot verify they were bone-length normalized the same way the
# video-derived features are. Silently mixing scaled and unscaled position
# data would bias exactly the comparison this pipeline exists to make.
KNEE_ONLY_CSV_ALIASES = {
    "right_knee_filtered": ["right_knee_filtered", "right_knee_angle"],
    "left_knee_filtered": ["left_knee_filtered", "left_knee_angle"],
}


def load_csv_skater_features(csv_path):
    """Loads a pre-extracted skater CSV and returns a standardized DataFrame
    containing ONLY the columns that are safe to compare across skaters
    without bone-length scaling (knee angles). Returns None if the CSV
    doesn't have at least one usable column.
    """
    try:
    ...
    found_any = False
    for target_col, aliases in KNEE_ONLY_CSV_ALIASES.items():
        for alias in aliases:
            if alias in df.columns:
                standardized[target_col] = df[alias]
                found_any = True
                break

    return standardized if found_any else None


def get_or_compute_skater_features_any(skater_name, video_map, csv_map, root_dir, force_recompute=False):
    """Tiered lookup: real video (full bone-scaled feature set) > CSV
    fallback (knee-angle only, no bone-scaling applied) > unavailable.

    Returns (df_features, reference_scale_or_None, source) where source is
    one of "video", "csv", or "none" -- callers should surface this to the
    user so it's clear which comparisons have the full feature set with
    bone-scaling applied vs. a reduced, unscaled-but-scale-invariant subset.
    """
    df, scale = get_or_compute_skater_features(skater_name, video_map, root_dir, force_recompute=force_recompute)
    if df is not None:
        return df, scale, "video"

    csv_rel = csv_map.get(skater_name)
    if csv_rel:
        full_csv = csv_rel if os.path.isabs(csv_rel) else os.path.join(root_dir, csv_rel)
        if os.path.exists(full_csv):
            df_csv = load_csv_skater_features(full_csv)
            if df_csv is not None:
                return df_csv, None, "csv"
