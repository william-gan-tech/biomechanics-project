...
# ==========================================
  ...
  st.markdown(
      "Choose whether to **upload an MP4 file** or **paste a YouTube link** to"
      " automatically run your pipeline, stride segmentation, and **INT8"
      " quantized ONNX runtime edge optimization** with cross-subject bone normalization."
  )

  input_method = st.radio(
      "Select Input Method", ["Upload MP4 File", "Paste YouTube URL"]
  )
  ...
  if st.session_state.pipeline_ran:
    with st.spinner(
        "Analyzing biomechanics, extracting keypoints, applying bone normalization, "
      ...
      try:
        ...
        # NOTE: If your pipeline function handles raw video path internally, 
        # ensure it calls normalize_skeleton_sequence on extracted tracking arrays.
        # Alternatively, if run_full_fatigue_pipeline accepts preprocessed arrays:
        # raw_keypoints = extract_keypoints(temp_path)
        # normalized_keypoints = normalize_skeleton_sequence(raw_keypoints)
        # result = run_full_fatigue_pipeline_with_data(normalized_keypoints)
        ...
        if st.button("Auto-Calibrate Threshold from First 5s"):
          if not df_rolling.empty and "timestamp_sec" in df_rolling.columns:
            ...
            if not baseline_slice.empty:
              ...
              st.success(
                  f"Calibrated dynamic threshold set to: {calibrated_val:.4f}"
