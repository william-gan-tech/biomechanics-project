...
def process_skating_video_multivariate(video_path, fps=30.0):
    # Get the absolute path of the 'src' directory, then go up one level to project root
...
if __name__ == "__main__":
    ...
    # --- 1. Process existing time trial video (Optional) ---
    ...
    # --- 2. Process Haralds Silovs technique video ---
    silovs_video = "data/silovs.mp4"
    if os.path.exists(silovs_video):
        print("\nProcessing Haralds Silovs technique video...")
        df_silovs = process_skating_video_multivariate(silovs_video)
        
        if df_silovs is not None:
            df_silovs.to_csv("data/haralds_silovs_baseline.csv", index=False)
            print(f"✅ Success! Saved {len(df_silovs)} frames -> data/haralds_silovs_baseline.csv")
    else:
        print(f"⚠️ Warning: Could not find {silovs_video}. Make sure silovs.mp4 is inside your data folder.")