...
class Encoder(nn.Module):
  ...
  def forward(self, x):
    ...
    return hidden[
        -1
    ]  # Extract final hidden state representing compressed latent space
...
class SkatingLSTMAutoencoder(nn.Module):
  ...
  def __init__(self, seq_len, n_features, embedding_dim=64, num_phases=3):
    super(SkatingLSTMAutoencoder, self).__init__()
    self.encoder = Encoder(seq_len, n_features, embedding_dim)
    self.decoder = Decoder(seq_len, n_features, embedding_dim)

    # Auxiliary classification head for skating phases (Push-off, Glide, Recovery)
  ...
  def forward(self, x):
    ...
    phase_logits = self.phase_classifier(latent)
    return reconstructed, phase_logits