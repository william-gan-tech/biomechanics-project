...
* **ONNX Edge Runtime Acceleration:** Exported the finalized PyTorch LSTM autoencoder into an optimized ONNX format (`skating_model.onnx`), utilizing `ort.InferenceSession` with explicit dynamic axes for batch size and sequence length to lower CPU/GPU inference latency.
